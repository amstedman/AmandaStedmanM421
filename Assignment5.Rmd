...
```{r}
summary(df$Age)
df$AgeGroups <- case_when(df$Age<=22 ~ 'Young', df$Age<=35 ~ 'Middle', df$Age>35 ~ 'Old' )
df %>% group_by(AgeGroups) %>% summarise(mean(Survived))
```
...
```{r}
df2 <- read_csv('https://bryantstats.github.io/math421/data/Vehicle.CSV')
...
mean(d$TRAV_SP, na.rm = TRUE)
average_speed <- mean(d$TRAV_SP, na.rm = TRUE)
d$TRAV_SP <- replace_na(d$TRAV_SP, average_speed)
table(d$TRAV_SP)
```
